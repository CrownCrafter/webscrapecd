...
startno = int(input("Start from: "))
numqu = int(input("Number of results: "))
def contact_finder(query, startno, numqu):
    url = []
    for j in search(query, num=10,pause=2, start = startno, stop = startno + numqu):
        url.append(j)
    title = []
    emails = []
    #numbers = []
...
contact_finder(query, startno, numqu)